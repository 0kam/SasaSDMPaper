...
\urlstyle{same}
\hypersetup{
  pdftitle={Snowmelt-driven shifts of a range-expanding plant in alpine ecosystems revealed by ground-based time-lapse cameras},
...
\begin{frontmatter}
\title{Snowmelt-driven shifts of a range-expanding plant in alpine
ecosystems revealed by ground-based time-lapse cameras}
...
Climate change is reorganizing alpine vegetation worldwide. However, the
processes linking snow dynamics to fine-scale vegetation redistribution
remain poorly quantified. We ask whether spatial variation in snowmelt
timing can explain and predict the expansion of competitively dominant
plants in alpine landscapes.
...
We combined ground-based time-lapse camera observations with habitat
suitability models (HSMs) to reconstruct past and project future
distributions of dwarf bamboo (\emph{Sasa} spp.) at 1-meter spatial
resolution. Time-lapse images collected from 2012 to 2021 were used to
generate automated vegetation maps and pixel-level estimates of snowmelt
timing. These variables were incorporated as predictors in the models.

\subsection*{Results}\label{results}
\addcontentsline{toc}{subsection}{Results}

Between 2012 and 2021, \emph{Sasa} cover increased by 48\%. Over the
same period, snowmelt timing advanced by an average of 8.6 days per
decade. Variable-importance analyses identified snowmelt day of year
(DOY) and distance from the prior \emph{Sasa} distribution as the
strongest determinants of current patterns. This indicates that both
climatic forcing and dispersal constraints shape distributional change.
Scenario-based projections under continued snowmelt advance suggested
further expansion of \emph{Sasa} by 2030.

\subsection*{Main conclusions}\label{main-conclusions}
\addcontentsline{toc}{subsection}{Main conclusions}

Our results demonstrate that snowmelt timing is a key biogeographic
driver structuring the spatial redistribution of dominant alpine plants.
They also show that fine-scale snow--vegetation relationships can be
quantified and projected using ground-based observations. By linking
climate-driven processes to vegetation patterns at meter resolution,
this study advances understanding of how alpine plant distributions
respond to changing cryospheric conditions. The approach provides a
transferable basis for assessing climate-induced vegetation
reorganization in snow-dominated ecosystems, with direct relevance for
conservation planning.

\section{Introduction}\label{introduction}

Alpine ecosystems are among the most vulnerable to climate change
(\citet{IPCCSR2019HM}). Across mountain regions worldwide,
climate-driven shifts in vegetation have been documented
(\citet{Gottfried2012NatClimChange}), including changes in timberline
positions and the spatial reorganization of alpine plant communities.
Such shifts reflect biogeographic responses to environmental gradients
and vary regionally depending on geological context, species
interactions, and disturbance regimes (\citet{Grabherr2010GeoComp},
\citet{Klanderud2005JEcol}).

A key mechanism underlying these biogeographic shifts is the emergence
or expansion of competitively dominant species, which can rapidly
reorganize community composition by excluding subordinate taxa. For
example, grass-dominated snowbed communities (\citet{Grabherr2003book})
and shrub encroachment into alpine grasslands (\citet{Dullinger2003AAA})
illustrate how dominance hierarchies mediate vegetation change across
alpine landscapes. In Japanese alpine ecosystems, dwarf bamboo
(\emph{Sasa} spp., hereafter \emph{Sasa}) represents a pronounced
example of such dominance. Earlier snowmelt associated with climate
change promotes \emph{Sasa} invasion into snowbed communities, resulting
in substantial reductions in plant species richness
(\citet{Kudo2011EcoEvo}). Conversely, vegetation recovery following
\emph{Sasa} removal has been documented (\citet{Kudo2017AlpBot}),
highlighting the importance of understanding how climate-driven
processes regulate the spatial dynamics of dominant alpine plants for
conservation.

Despite their importance, fine-scale biogeographic processes in alpine
ecosystems remain difficult to quantify. Alpine regions are remote and
topographically rugged, making traditional vegetation surveys costly and
logistically demanding. Satellite-based observations are also limited by
frequent cloud cover and the fine-grained, mosaic structure of alpine
vegetation. These constraints hinder our ability to link environmental
drivers, such as snowmelt timing, to observed vegetation patterns at
ecologically meaningful scales.

To address these challenges, a method for generating high-resolution
vegetation classification maps using ground-based time-lapse cameras has
been developed (\citet{Okamoto2024RSEC}). This approach exploits
temporal changes in vegetation colour captured by fixed cameras and
applies precise georectification to align ground-based imagery
spatially. By combining high temporal frequency with fine spatial
resolution, time-lapse cameras provide an opportunity to quantify
climate-driven biogeographic processes that are difficult to observe
using conventional approaches.

Here, we apply this framework to a Japanese alpine system as a model
case to examine snowmelt-driven vegetation dynamics. Specifically, we
quantified \emph{Sasa} expansion into alpine meadows on the western
slope of Mt. Tateyama in the northern Japanese Alps. In the nearby
Murodo-daira plateau, \emph{Sasa} species (\emph{Sasa kurilensis},
\emph{Sasa spiculosa}, \emph{Sasa senanensis}, and \emph{Sasa palmata})
expanded by 44--260\% between 1977 and 2015
(\citet{Yoshida2016SasaTateyama}). Previous studies relied primarily on
field surveys or manual interpretation of aerial photographs, limiting
spatial resolution and temporal continuity. By contrast, automated
processing of time-lapse imagery enabled us to generate large-scale,
high-resolution maps of \emph{Sasa} distribution across the study area.

A particular advantage of time-lapse cameras is their capacity to
capture frequent, site-specific records of snowmelt timing, a
fundamental driver of alpine vegetation dynamics. Leveraging this
capability, we modelled \emph{Sasa} distributions derived from
time-lapse imagery using environmental variables such as snowmelt timing
and topographic factors, and predicted distributional shifts under
scenarios of earlier snowmelt. Habitat suitability models
(HSMs)---statistical frameworks relating species occurrence to
environmental conditions---are widely used to elucidate habitat
preferences (\citet{Guisan1998JVS}), inform conservation planning
(\citet{Larson2004EcolModel}, \citet{Xue2021GEC}), forecast biological
invasions (\citet{Gallien2010DD}, \citet{Gallien2012GEB}), and assess
range shifts and extinction risks under climate change
(\citet{Thomas2004Nature}, \citet{Bakkenes2002GCB},
\citet{Amagai2022AVS}).

Using HSMs constructed from observed \emph{Sasa} distributions and their
temporal changes, we addressed two primary questions: (1) whether
snowmelt timing acts as a critical determinant of dominant plant
distributions in alpine landscapes, and (2) how such distributions may
shift under future conditions of earlier snowmelt. By linking
high-resolution observations with predictive modelling, this study
demonstrates how ground-based time-lapse imagery can be used to detect
and forecast climate-driven vegetation redistribution, with implications
for conservation monitoring and management in alpine ecosystems.

\section{Materials and Methods}\label{materials-and-methods}

Although demonstrated in a Japanese alpine system, the workflow
presented here is applicable to any snow-dominated ecosystem monitored
by fixed ground-based cameras, including existing alpine webcam networks
such as those established in the Swiss Alps
(\citet{Portenier2020Cryosphere}). The approach is designed as a
biogeographic inference pipeline that links climate-driven processes,
represented by snowmelt timing, to vegetation patterns and their
redistribution under environmental change.
...
with the \emph{Sasa} distribution and thus had little impact on our
analysis. Additional details on snowmelt timing and its prediction are
provided in the supplementary information. In the TDM, distance from the
2012 \emph{Sasa} distribution was included as an additional explanatory
variable to represent dispersal limitation.
...
This study demonstrates how snowmelt-driven processes can be quantified
and projected at fine spatial scales using ground-based time-lapse
imagery integrated with habitat suitability models (HSMs). Using a
Japanese alpine system as a model case, we assessed recent changes and
potential future expansion of a competitively dominant plant
(\emph{Sasa}) at an exceptionally high spatial resolution (1 m). Beyond
documenting local vegetation change, our results highlight how snowmelt
timing acts as a key biogeographic driver structuring alpine vegetation
patterns and dynamics.

\subsection{Snowmelt timing as a driver of dominant plant
expansion}\label{snowmelt-timing-as-a-driver-of-dominant-plant-expansion}
...
results indicate that earlier snowmelt has facilitated the expansion of
a competitively dominant plant, consistent with previous findings
(\citet{Kudo2011EcoEvo}) linking earlier snowmelt to increased soil
dryness and extended growing periods that favor \emph{Sasa} growth.
...
models that explicitly simulate dispersal dynamics could help clarify
whether the overestimation results from dispersal limitation or omitted
environmental factors.

\subsection{Methodological implications for conservation
biogeography}\label{methodological-implications-for-conservation-biogeography}

This study demonstrates that combining time-lapse cameras with HSMs
enables exceptionally fine-scale (1 m) prediction of vegetation
distribution changes driven by climate-related processes. The
integration of time-lapse imagery and HSMs is unprecedented. In
contrast, conventional species distribution models typically operate at
much coarser resolutions (e.g., 1 km), as future climate projections are
generally available only at coarse spatial scales (e.g., alpine
vegetation studies: \citet{Amagai2022AVS}). By directly observing
snowmelt timing---a key determinant of alpine vegetation
dynamics---using time-lapse cameras, we achieved high-resolution
predictions that would be difficult to obtain using satellite-based or
plot-based approaches alone.

Because snowmelt timing requires both frequent and spatially extensive
monitoring, time-lapse cameras are particularly well suited for
capturing this biogeographic driver. This approach provides a pathway
for linking climate-driven processes to spatial vegetation patterns in
alpine and other snow-dominated ecosystems where conventional monitoring
methods are limited.

\subsection{Future projections and scenario-based
interpretation}\label{future-projections-and-scenario-based-interpretation}

In this study, future snowmelt timing was predicted using a simple
linear regression based on a decade of past observations. While this
approach provided a useful proof of concept, it does not represent a
realistic long-term forecast. In alpine regions, strong seasonal winds
and complex topography cause substantial snow redistribution, making
accurate snowmelt prediction a persistent challenge. Accordingly, the
future \emph{Sasa} expansion presented here should be interpreted not as
a literal forecast but as a scenario based on the assumption of
continued earlier snowmelt. As high-resolution climate and snow models
improve, this framework can readily incorporate more realistic
projections, thereby refining predictions of vegetation redistribution.

\subsection{Conservation implications}\label{conservation-implications}

The expansion of competitively dominant species such as \emph{Sasa}
poses a serious threat to alpine biodiversity by reducing habitat
availability for subordinate and often rare alpine plants.
High-resolution predictions of vegetation change, as demonstrated here,
provide a practical basis for conservation management by identifying
priority areas for intervention, such as targeted \emph{Sasa} removal,
and for designing efficient long-term monitoring strategies. More
broadly, our results emphasize that snowmelt timing is a critical
process shaping alpine vegetation patterns and that fine-scale,
process-based monitoring can substantially enhance conservation planning
in climate-sensitive mountain ecosystems.
...
The data and scripts supporting this study are openly available in an
anonymous public repository for peer review at:
\url{https://anonymous.4open.science/r/SasaSDMPaper-67CE/}
