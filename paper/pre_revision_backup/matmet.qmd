...
To quantify how snowmelt timing influences *Sasa* distribution and to project its potential expansion under changing conditions, we constructed habitat suitability models (HSMs). While vegetation maps derived from time-lapse imagery provide high-resolution snapshots of current distribution, they do not explicitly quantify the relationship between environmental variables and species occurrence. HSMs were therefore used to infer these relationships and to predict distributional responses under scenarios of earlier snowmelt.
...
#| out-width: 100%
#| fig-align: center
knitr::include_graphics("files/overview.jpg")
```
...
#| out-width: 100%
#| fig-align: center
c(image_read("files/2012_5x5_en.jpg"), image_read("files/2021_5x5_en.jpg")) |>
  image_append() |>
  image_resize("2400x") |>
  image_write("files/vegemap.jpg")

knitr::include_graphics("files/vegemap.jpg")
```
...
We focused on snowmelt day of year (DOY) as a key predictor because it directly determines the onset and duration of the growing season in alpine ecosystems. Although other snow-related metrics (e.g., snow fall amount) may also influence vegetation dynamics, these variables are typically strongly correlated with snowmelt timing and are difficult to estimate reliably at fine spatial scales from time-lapse imagery.
...
#| out-width: 100%
#| fig-align: center
c(image_read("files/initial_split_dist.jpg"), image_read("files/cv_dist.jpg")) |>
  image_append() |>
  image_resize("2400x") |>
  image_write("files/initial_split.jpg")
knitr::include_graphics("files/initial_split.jpg")
```
