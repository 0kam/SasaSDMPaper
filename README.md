...
Large binaries are excluded from git (see `.gitignore`). The inputs needed to re-run
stage 4 are deposited on Zenodo (concept DOI https://doi.org/10.5281/zenodo.22745234,
which resolves to the latest version), together with a snapshot of this repository:

| Deposited file(s) | Place at |
|---|---|
| `vege_2012_5x5.tiff`, `vege_2021_5x5.tiff` | `ortho/data/` |
| `MRD_snowfront_L_*.tiff` (10 files, 2011–2021) | `ortho/data/snow/raw/` |
| `tateyamadem_small.tif`, `slope.tif`, `aspect.tif`, `TPI.tif`, `twi.tif`, `TRI.tif`, `roughness.tif` | `ortho/data/terrain_features/` |
| `mrd_dem_1m.tiff`, `georectified.tiff`, `dem_small.tiff` | `ortho/data/` (georectification inputs) |

The Zenodo record stores files as a flat list; file names are unique across the
repository, so each file can be placed back at the path above before running
`analysis/01_predictors.R` onwards.

Not deposited, because they are regenerated by the pipeline from the files above:
`analysis/out/models/*.rds` (fitted Model A ensembles and Model B GAMs), `analysis/out/*.tif`
(predictor stack, distance surface, suitability, establishment-probability and projection
rasters), and `review/image_interpretation_package/{crops,crops_native,blind_overlay}/`
(image crops, rebuilt by `sample_loss_crops.py` and `make_blind_overlays.py`). Also not
deposited: `results/*.npy` (image-space classifier output, regenerated by stage 1),
`data_external/veg2024bk4.gpkg` (the public Ministry of the Environment source of
`veg_murodo.gpkg`), and the georectification intermediates `georectified.csv`,
`pointcloud.db` and `tateyama2.tiff`, which are held by the authors and available on request.
